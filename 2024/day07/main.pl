...
sub check_target{
  ...
  $summ = check_target($target, $current_result + $num, $index+1, $nums, $is_part1, $memo);
  if ($index == 0) {
    $multi = check_target($target, $num, $index + 1, $nums, $is_part1, $memo);
  } else {
    $multi = check_target($target, $current_result * $num, $index + 1, $nums, $is_part1, $memo);
  }
  if ($is_part1) {
    $concat = check_target($target, $current_result . $num, $index + 1, $nums, $is_part1, $memo);
  }
